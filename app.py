...
# Type-based fallback images (reliable Wikimedia Commons URLs)
TYPE_FALLBACK_IMAGES = {
    ...
    "Religious":    "https://upload.wikimedia.org/wikipedia/commons/thumb/9/94/Golden_Temple_amritsar.jpg/800px-Golden_Temple_amritsar.jpg",
    "Beach":        "https://upload.wikimedia.org/wikipedia/commons/thumb/6/60/Anjuna_beach_Goa_India.jpg/800px-Anjuna_beach_Goa_India.jpg",
    "Adventure":    "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1e/Pangong_tso.jpg/800px-Pangong_tso.jpg",
    "Nature":       "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b1/Munnar_tea_estate.jpg/800px-Munnar_tea_estate.jpg",
    "Wildlife":     "https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/Single_Rumput.jpg/800px-Single_Rumput.jpg",
    "Hill Station": "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Shimla_view_01.jpg/800px-Shimla_view_01.jpg",
    "Cultural":     "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Mysore_Palace_illuminated.jpg/800px-Mysore_Palace_illuminated.jpg",
    "default":      "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1d/Taj_Mahal_%28Edited%29.jpeg/800px-Taj_Mahal_%28Edited%29.jpeg",
}

@st.cache_data(show_spinner=False)
def get_wiki_image(dest_name: str, dest_type: str = "") -> str:
    """
    Fetch Wikipedia thumbnail for a destination name.
    Falls back to type-based Wikimedia image if not found.
    """
    search_names = [dest_name, dest_name.split()[0]]
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if thumb:
                    ...
                    thumb = thumb.replace("/320px-", "/800px-").replace("/200px-", "/800px-")
                    return thumb
        except Exception:
            pass
    # Fallback: destination-type based image
    return TYPE_FALLBACK_IMAGES.get(dest_type, TYPE_FALLBACK_IMAGES["default"])

# =========================================================
# PAGE CONFIG
# =========================================================
st.set_page_config(
    page_title="Incredible India Explorer – AI-Powered Travel Recommender System",
    page_icon="✈️🇮🇳",
    layout="wide"
)
...
def parse_average_cost(cost_value):
    """Convert average cost string/number to numeric value"""
    if pd.isna(cost_value):
        return 0
    if isinstance(cost_value, (int, float)):
        val = int(cost_value)
        return val if val < 1_000_000 else 50000
    cost_str = str(cost_value).replace("₹", "").replace(",", "").strip()
    # Handle range like "5000-12000" → take average of low and high
    if "-" in cost_str:
...
def destination_card(row, show_recommend_btn=False, cosine_sim=None, indices=None, destinations=None):
    ...
    dest_name = str(row.get("Name", ""))
    dest_type = str(row.get("Type", ""))

    # Check if CSV URL is a real direct image (wikimedia upload, jpg/png CDN, etc.)
    # If it's a search page or deprecated unsplash, fetch from Wikipedia instead
    is_direct_image = (
        isinstance(img_url, str)
        and img_url.strip() != ""
        and "google.com/search" not in img_url
        and "source.unsplash.com" not in img_url
        and img_url.strip().lower() not in ["nan", "none", ""]
    )

    if is_direct_image:
        final_img_url = img_url.strip()
    else:
        # Fetch Wikipedia thumbnail (cached per destination name)
        final_img_url = get_wiki_image(dest_name, dest_type)

    st.markdown('<div class="img-container">', unsafe_allow_html=True)
    try:
        st.image(final_img_url, use_container_width=True)
    except Exception:
        # If even that fails, use type-based fallback
        fallback = TYPE_FALLBACK_IMAGES.get(dest_type, TYPE_FALLBACK_IMAGES["default"])
...
def main():
    ...
    <div class="main-header">
        ...
        <p>AI-Powered Indian Travel Recommender System</p>
    </div>
    """, unsafe_allow_html=True)
